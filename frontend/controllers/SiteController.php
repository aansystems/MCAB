...
class SiteController extends Controller {
    ...
    public function actionIndex() {
        if (Yii::$app->user->isGuest) {
            ...
            if ($role_type == 1) {
                ...
                $query_nactive = "SELECT id FROM `user` WHERE `role_type` = 4 AND (`added_by` = 0 AND `created_by` = " . Yii::$app->user->identity->id . " OR `added_by` = 1) AND `status` = 0";
            ...
            } else if ($role_type == 4 || $role_type == 8 || $role_type == 9) {
    ...
    public function actionDashboardLearner() {
        if (Yii::$app->user->isGuest) {
            ...
            if ($role_type == 4 || $privildge == 1 || $role_type == 8 || $role_type == 9) {
